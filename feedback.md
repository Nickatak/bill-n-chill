# PM Feedback — Quote Creator (2026-03-28)
...
2. [x] **Sub Items** — Hierarchical line items (parent/child nesting for visual grouping). *Moderate.*
    - Implemented via sections: named section headers with drag-and-drop reordering, subtotals per section. Live on quotes and COs.
3. [x] **Drag and Drop Reorder** — Desktop only via @dnd-kit. Line number + grip handle as drag target. Mobile keeps up/down arrows. *(Done — quotes.)*
4. [ ] **Contract Attachment** — Store a link to external contract/DocuSign on the quote. *Trivial.*
5. [ ] **Bottom of Bid Markups** — Summary-level markups below line items (contingency, OH&P, insurance). *Moderate.*
6. [x] **General Notes / Exclusions** — Text area at bottom of quote for scope notes, exclusions, caveats. *(Done.)*
    - Added general notes/exclusions editable text box.
7. [x] **Custom Quote Number** — Optional user-defined quote number per job (e.g. job name + number). *Small.*
    - Remove the Quote N# display: that's the internal DB ID, it's not controlled by the user.
...
9. [ ] **Billing Schedule** — Payment schedule by % and amount with details, on quotes/invoices. *Larger — needs child model + UI.*
