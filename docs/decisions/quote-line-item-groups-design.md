...
- `ordering = ["order"]`
- `unique_together = ("quote", "order")`

### Changes to `QuoteLineItem`

| Field   | Type                                      | Notes                          |
|---------|-------------------------------------------|--------------------------------|
| `group` | FK(QuoteLineItemGroup, SET_NULL, null)  | Nullable — ungrouped if null   |
...
- **Group order is unique per quote** — DB enforced via `unique_together`.
...
- Groups and line items returned separately in the quote response
...
1. **Ungrouped items render above groups** — default position is top, so an quote with no groups looks identical to today.
2. **Shared ordering space** — groups and ungrouped line items interleave freely. A single `order` value on both models determines position at the quote level. Group A, then two loose items, then Group B is a valid layout. Reorder logic is uniform: everything is a row with a position.
