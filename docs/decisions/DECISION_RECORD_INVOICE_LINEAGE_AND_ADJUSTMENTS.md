...
- We need invoice flexibility for real-world billing operations, including non-quote charges.
- We also need strict financial traceability and auditability over time.

## Rationale

1. `QuoteLineItem` rows are context rows, not canonical identity.
2. Invoices are billing-time artifacts that may regroup/split/partially bill prior scope.
3. Directly anchoring invoice lines to mutable quote rows would over-couple invoice workflows.
...
- Traceability:
  - Lines can be categorized via cost codes across quote and invoice artifacts.
