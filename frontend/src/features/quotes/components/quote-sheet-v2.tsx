/**
 * Quote sheet v2 — responsive line items with flat ordering and sections.
 *
...
import { BillingPeriodInput, CostCode, QuoteLineInput, QuoteSectionRecord, ProjectRecord } from "../types";
...
export type QuoteSheetV2Handle = {
  getOrderPayload: () => OrderPayload;
};

type QuoteSheetV2Props = {
...
export const QuoteSheetV2 = forwardRef<QuoteSheetV2Handle, QuoteSheetV2Props>(
  function QuoteSheetV2(
    {
      project,
    ...
    const titleReadOnly = readOnly || isExistingQuote || titleLocked;
    ...
    // Hydrate sections from API response when the quote or its sections change.
    ...
    useEffect(() => {
      const hydrateKey = `${quoteId}::${apiSectionsFingerprint}`;
      ...
      hydrationPending.current = true;
    }, [quoteId, apiSectionsFingerprint, apiSections, lineItems]);
    ...
    type Entry =
      ...
      | { type: "item"; line: QuoteLineInput; index: number };
    ...
    function renderLineItemRow(line: QuoteLineInput, index: number, handleProps: Record<string, unknown>) {
      ...
      <section className={creatorStyles.sheetSection}>
        <div className={creatorStyles.sheetHeader}>
          ...
          <div className={creatorStyles.headerRight}>
            ...
            <div className={creatorStyles.sheetTitle}>Quote</div>
      ...
      <section className={creatorStyles.sheetSection}>
        <div className={creatorStyles.metaOnlyRow}>
          <div className={creatorStyles.metaBlock}>
            <div className={creatorStyles.metaTitle}>Quote Details</div>
            ...
            <div className={creatorStyles.metaLine}>
              ...
              <input className={creatorStyles.fieldInput} type="date" value={quoteDate} disabled aria-disabled="true" />
            </div>
            <div className={creatorStyles.metaLine}>
    ...
    const termsSection = (
      <section className={creatorStyles.sheetSection}>
        <div className={creatorStyles.terms}>
          ...
          {(termsText || organizationDefaults?.quote_terms_and_conditions || "Not set")
            .split("\n")
            .filter((line) => line.trim())
    ...
    const footerSection = (
      <section className={creatorStyles.sheetSection}>
        <div className={creatorStyles.footer}>
          ...
          <span>{quoteId ? "Quote" : "Draft quote"}</span>
        </div>
      </section>
    );
        ...
        <div className={creatorStyles.finalizeActions}>
          ...
          <button type="submit" className={creatorStyles.primaryButton} disabled={!canSubmit || isSubmitting}>
            {isSubmitting
              ...
              : isEditingDraft ? "Save Draft Changes" : "Create Quote"}
          </button>
        </div>
      </section>
    ) : null;
