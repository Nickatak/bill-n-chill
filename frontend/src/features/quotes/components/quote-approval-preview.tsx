...
/**
 * Public-facing quote approval preview for customer decision flow.
 * Renders a read-only quote document via a tokenized public URL and
 * provides approve/reject controls for quotes in "sent" status.
 *
 * Parent: app/quote/[publicRef]/page.tsx
 */
...
import { ApiResponse, BillingPeriodInput, QuoteRecord } from "../types";
...
type QuoteApprovalPreviewProps = {
  publicToken: string;
};

/** Renders the public quote preview and customer decision form. */
export function QuoteApprovalPreview({ publicToken }: QuoteApprovalPreviewProps) {
  const [statusMessage, setStatusMessage] = useState("Loading quote...");
  ...
  const lineItems = useMemo(() => mapPublicQuoteLineItems(quote), [quote]);
  const costCodes = useMemo(() => mapLineCostCodes(quote), [quote]);
  const quoteDate = formatDateInputFromIso(quote?.created_at);
  const validThrough = quote?.valid_through ?? "";
  const sender = useMemo(
    () => resolvePublicSender(quote?.organization_context, quote),
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentionally granular deps
    [quote?.organization_context, quote?.sender_name, quote?.sender_address, quote?.sender_logo_url],
  );
  const recipient = useMemo(
    () => resolvePublicRecipient(quote?.project_context),
    [quote?.project_context],
  );
  const termsText = useMemo(() => {
    const documentTerms = (quote?.terms_text || "").trim();
    if (documentTerms) return documentTerms;
    const organizationTerms = resolveDefaultTerms(quote?.organization_context, "quote");
    return organizationTerms || "No terms specified.";
  }, [quote?.organization_context, quote?.terms_text]);
  ...
  const contingencyPercent = String(quote?.contingency_percent ?? "0");
  const contingencyAmount = subtotal * (Number(contingencyPercent) / 100);
  const overheadProfitPercent = String(quote?.overhead_profit_percent ?? "0");
  const overheadProfitAmount = subtotal * (Number(overheadProfitPercent) / 100);
  const insurancePercent = String(quote?.insurance_percent ?? "0");
  const insuranceAmount = subtotal * (Number(insurancePercent) / 100);
  const taxPercent = String(quote?.tax_percent ?? "0");
  ...
  const billingPeriods: BillingPeriodInput[] = useMemo(() => {
    const records = quote?.billing_periods ?? [];
  ...
  }, [quote?.billing_periods]);

  const canDecide = quote?.status === "sent";
  const hasDecision = quote?.status === "approved" || quote?.status === "rejected";
  const decisionStatusLabel = quoteStatusLabel(quote?.status);
  // Clear stale decision feedback when the quote is no longer actionable.
  useEffect(() => {
    if (!canDecide) {
      setDecisionMessage("");
    }
  ...
  // Fetch the quote record from the public token on mount.
  useEffect(() => {
    async function loadQuoteContext() {
      try {
        const quoteRes = await fetch(`${normalizedBaseUrl}/public/quotes/${publicToken}/`);
        const quoteJson: ApiResponse = await quoteRes.json();
        if (!quoteRes.ok || !quoteJson.data) {
          setStatusMessage("Quote not found.");
          return;
        }

        const nextQuote = quoteJson.data as QuoteRecord;
        setQuote(nextQuote);
        setDecisionReceiptName("");
        setStatusMessage("");
      } catch {
        setStatusMessage("Could not reach quote endpoint.");
      }
    }
  ...
  async function applyDecision(decision: string, ceremony: CeremonyPayload) {
    if (!quote || !canDecide || decisionSubmitting) {
      return;
    }
    ...
    try {
      const response = await fetch(`${normalizedBaseUrl}/public/quotes/${publicToken}/decision/`, {
        method: "POST",
      ...
      const nextQuote = payload.data as QuoteRecord;
      ...
      setDecisionMessage("Could not reach quote decision endpoint.");
    } finally {
    ...
    >
      {quote ? (
        <>
          <PublicDocumentFrame
            ...
            headerRight={
              <>
                ...
                <div>
                  <p className={frameStyles.identityEyebrow}>Quote</p>
                  <h2 className={frameStyles.identityTitle}>{quote.title || "Untitled"}</h2>
                  <p className={frameStyles.identitySubhead}>
                    {(quote.project_context?.name || "Project") + ` · v${quote.version}`}
                  </p>
                </div>
                <hr className={frameStyles.identityDivider} />
                <section className={`${frameStyles.metaDetails} ${styles.publicDetailsPanel}`}>
                  <h4 className={frameStyles.metaDetailsTitle}>Quote Details</h4>
                  ...
                  <div className={frameStyles.metaDetailsRow}>
                    ...
                    <span>{formatDateDisplay(quoteDate, "Not set")}</span>
                  </div>
                  <div className={frameStyles.metaDetailsRow}>
            ...
            rows={(() => {
              const sections = [...(quote.sections ?? [])].sort((a, b) => a.order - b.order);
                ...
                {quote.notes_text ? (
                  <div className={frameStyles.terms}>
                    ...
                    {quote.notes_text
                      ...
                      .map((line, index) => (
                        <p key={`quote-notes-${index}`}>{line}</p>
                      ))}
                  </div>
                ) : null}
                <div className={frameStyles.terms}>
                  ...
                  {termsText
                    ...
                    .filter((line) => line.trim())
                    .map((line, index) => (
                      <p key={`quote-terms-${line}-${index}`}>{line}</p>
                    ))}
                </div>
              </>
            }
            footer={
              <footer>
                <div className={frameStyles.footerRow}>
                  ...
                  <span>{quote.public_ref || publicToken}</span>
                </div>
          ...
          />
          {canDecide ? (
            <div ref={decisionSectionRef} id="quote-decision">
              ...
              <SigningCeremony
                ...
                customerEmailAvailable={Boolean(quote.project_context?.customer_email)}
                consentText={quote.ceremony_consent_text ?? ""}
                decisions={[
                  { label: "Approve Quote", value: "approve", variant: "primary" },
                  { label: "Reject Quote", value: "reject", variant: "secondary" },
                ]}
          ...
          {hasDecision ? (
            <div
              ...
              className={`${stampStyles.decisionStamp} ${
                quote?.status === "approved" ? stampStyles.decisionStampApproved
